...
#sent_1 = "#define rfloat(x) scanf(%lf, &(x))?"
...
with open("train/kaleem/welcome.txt", 'r', errors='ignore') as file:
   sent = file.read().replace('\n', '')

sent ="old dead peoples"
print(sent)
...
print(str(res)+" "+str(np.argmax(res))+" "+str(len(categories)))
print(res[0][np.argmax(res)])
print((str(np.argmax(res[0]))))

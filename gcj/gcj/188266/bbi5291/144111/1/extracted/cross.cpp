...
long long W[20][20];
long long T[20][20];
long long wait_NS(long long t,long long S,long long W,long long T)
{
	long long x=(t-T)%(S+W); if (x<0) x+=S+W;
	if (x<S) return 0;
	else return S+W-x;
}
long long wait_EW(long long t,long long S,long long W,long long T)
{
	long long x=(t-T)%(S+W); if (x<0) x+=S+W;
	if (x<S) return S-x;
	else return 0;
...
void dijkstra(loc_t src)
{
	...
	priority_queue<pair<long long,loc_t>,vector<pair<long long,loc_t> >,greater<pair<long long,loc_t> > > Q;
	Q.push(make_pair(0,src));
	while (Q.size())
	{
		pair<long long,loc_t> P=Q.top();
		loc_t L=P.second;
		Q.pop();

...
Matrix M(11, 10000000); // 1 = happy, 2 = unhappy, 3 = seen
...
double ComputeProb(int C, int N)
{
	double ev = 0.0;
	...
	while (k < C)
	{
		// Get next amount we want to select
		double amt = min(N, C - k);

		// Get number of steps to select it
		ev += amt * C / ((double) N * (C - k));
...
void Prob()
{
	...
	for (int cs = 1; cs <= T; ++cs)
	{
		...
		cout.precision(7);
		cout << "Case #" << cs << ": " << fixed << ComputeProb(C, N) << endl;
	}
}

...
map<set<int>, int64> smallest_happy;

bool is_happy(int64 num, int base) {

    //if (num < smallest_happy[base]) return false;
...
void docase(int64 case_number) {
    ...
    for (map<set<int>, int64>::iterator it = smallest_happy.begin(); it != smallest_happy.end(); ++it) {
        bool all_match = true;
        for (set<int>::iterator ita = (it->first).begin(); ita != (it->first).end(); ++ita) {
            bool found = false;
        ...
        if (all_match && (it->second > this_try)) this_try = it->second;
    }
    ...
    set<int> this_set_bases;
    for (size_t i = 0; i < bases.size(); ++i) {
        this_set_bases.insert(bases[i]);        
    }
    
    smallest_happy[this_set_bases] = this_try;

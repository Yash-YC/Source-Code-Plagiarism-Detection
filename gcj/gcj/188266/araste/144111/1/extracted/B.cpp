...
int N, M;
long long S[50][50], W[50][50], T[50][50];
long long D[10000];
bool checked[10000];
...
long long cross_ns(int r, int c, long long t) {
    int s = S[r][c], w = W[r][c];
    int period = s+w;
    long long nt = (t / period * period) + (T[r][c] % period) + 2*period;
...
long long cross_we(int r, int c, long long t) {
    int s = S[r][c], w = W[r][c];
    int period = s+w;
    long long nt = (t / period * period) + (T[r][c] % period) + 2*period + s;
    while (nt > t) nt -= period;
